    ...
    <div className="p-4 bg-white rounded-xl shadow-md border border-gray-200 flex flex-col gap-y-4">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500 mt-1">
            ...
            {uploadResponse?.fileUniqueId || "Pending"}
          </p>
        </div>
      </div>
      ...
      <div className="mt-5 grid grid-cols-5 gap-4 text-center">
        <div className="bg-blue-50 rounded-md py-3 shadow-sm">
          ...
          <p className="text-lg font-bold text-blue-600">{uploadResponse?.rowCount ?? 0}</p>
        </div>

        <div className="bg-green-50 rounded-md py-3 shadow-sm">
          <p className="text-sm text-gray-500">Success</p>
          <p className="text-lg font-bold text-green-600">{uploadResponse?.successCount ?? 0}</p>
        </div>

        <div className="bg-red-50 rounded-md py-3 shadow-sm">
          <p className="text-sm text-gray-500">Failed</p>
          <p className="text-lg font-bold text-red-600">{uploadResponse?.failedCount ?? 0}</p>
        </div>

        <div className="bg-yellow-50 rounded-md py-3 shadow-sm">
          <p className="text-sm text-gray-500">Duration</p>
          <p className="text-lg font-bold text-yellow-600">{uploadResponse?.duration || "Running"}</p>
        </div>

        <div className="bg-indigo-50 rounded-md py-3 shadow-sm">
          <p className="text-sm text-gray-500">Progress</p>
          <p className="text-lg font-bold text-indigo-600">{uploadResponse?.progress ?? 0}%</p>
        </div>
      </div>
    </div>
  );
